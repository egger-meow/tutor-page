...
│   └── effects.js          # 動效層：捲入動畫、證書燈箱、進度條（不用改）
├── assets/
│   ├── cv.pdf              # （自行放入）完整履歷
│   └── images/             # 占位圖片，同名覆蓋即可
└── README.md
...
價格寫在 `index.html` FAQ 區塊「收費怎麼算？」內。目前為：
- 試教：國小 300 元、國中 400 元
- 長期：國小 NT$500–600／小時、國中 NT$600–700／小時
- 高中物理、程式與 AI 類課程依內容再討論

請依實際定價修改，並同步更新頁面 meta description。
...
- 動效只做「加分不擋路」的層級（`scripts/effects.js`）：捲入浮現、證書點擊放大、進度條，
  微互動；全部尊重 `prefers-reduced-motion`，JS 失效時內容照常完整顯示。
  不加輪播、不加大型滾動特效（影像序列等），快與可信永遠優先。
